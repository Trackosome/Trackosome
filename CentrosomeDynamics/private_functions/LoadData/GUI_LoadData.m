...
[CM_filename, dirname] = uigetfile([searchPath '*.tif;*.tiff'],...
...
prompt   = {'centrosome radius in px',  'frame step [s]', 'z step [um]',...
    'px2um [\mum/px]', 'Nº Stacks', 'Nº Frames'};
...
try
    ...
    if (new_nFrames * new_nStacks) ~= (nStacks * nFrames)
        msgbox(['Nº Stacks x Nº Frames must be equal to: ', num2str(nStacks * nFrames)])
    else
    ...
    centrosome_radius_px = str2double( '6.0' ); %px
    ratio_z_xy_px = z_step/px2um;
end
...
manageButtons(handles, 3); % state 3 --> cropping
